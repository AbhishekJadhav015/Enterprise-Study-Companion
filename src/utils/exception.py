...
def error_message_detail(error, error_detail: sys):
    """"This function returns the error message with details or error such as file name line number"""
    _,_,exc_tb = error_detail.exc_info()
    file_name = exc_tb.tb_frame.f_code.co_filename
    error_message = "Error occured in python script name[{0} line number[{1}] error number[{2}]".format(file_name ,exc_tb.tb_lineno, str(error))
